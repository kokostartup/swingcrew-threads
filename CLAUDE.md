...
테스트 프레임워크 없음. `.env` 파일에 환경변수 설정 필요.
...
- `commands/*.md` — Claude Code 슬래시 커맨드 정의 파일 (`/threads-publish`, `/threads-generate` 등).

### Lib (`lib/`)
- `notion.py` — Notion API 래퍼. DB 쿼리(`query_db`), 페이지 본문 읽기(`get_page_content`), 속성 읽기/쓰기 헬퍼. Notion-Version: 2022-06-28.
- `threads.py` — Threads Graph API 래퍼. 컨테이너 생성 → 30초 대기 → 발행 패턴. `post_text`, `post_image`, `post_chain` 편의 함수. 인사이트 조회(`get_media_insights`, `get_threads_with_insights`).
- `teams.py` — MS Teams Incoming Webhook으로 Adaptive Card 전송.

### Skills (`skills/`)
- `threads-post-generator/SKILL.md` — 커맨드 라우터 정의.
- `threads-post-generator/references/style-guide.md` — 글 생성 시 참조하는 스타일 가이드.

### GitHub Actions (`.github/workflows/`)
- `threads_auto.yml` — 매일 5회(KST 7,12,17,20,22시) 승인된 글 1건씩 자동 게시. `workflow_dispatch`로 수동 실행 가능.
- `threads_report.yml` — 매일 오전 10시 KST 일일 리포트 → Teams 전송.
...
| `THREADS_ACCESS_TOKEN` | Meta Threads API 토큰 | publish, report |
...
게시 상태(select): 대기→승인→게시완료/실패. 게시 타입(select): 숏폼/롱폼/체인/골프소식/영상코멘트. 기타: 제목, 게시일, Threads URL, 이미지(files), 이미지 상태(select), 메모.
