...
def create_page(database_id, properties, content_text=None):
    """새 페이지를 DB에 생성. content_text가 있으면 본문 블록도 추가."""
    body = {
        "parent": {"database_id": database_id},
        "properties": properties,
    }
    if content_text:
        # 2000자 단위로 블록 분할 (노션 블록 텍스트 제한)
        chunks = _split_text(content_text, 2000)
        body["children"] = [
            {
                "object": "block",
                "type": "paragraph",
...
def get_prop_url(page, prop_name):
    """url 속성에서 URL 추출."""
    prop = page.get("properties", {}).get(prop_name, {})
...
def get_prop_relation(page, prop_name):
    """relation 속성에서 관련 페이지 ID 목록 추출."""
    prop = page.get("properties", {}).get(prop_name, {})
